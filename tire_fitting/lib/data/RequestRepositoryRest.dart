...
class RequestRepositoryRest extends RequestRepository {
  String entitiyUrl =
      'http://192.168.0.58:8080/app/rest/v2/entities/tirefitting_Request';
  ...
  Future<List<Request>> getRequests(ServicePoint servicePoint) async {
    try {
      http.Response response = await http.get(
          baseAddress +
              "/queries/tirefitting_Request/requestByServicePoint?id=" +
              servicePoint.id +
              "&view=request",
          headers: {'Authorization': 'Bearer ' + token});

      (jsonDecode(response.body) as List).forEach((element) {
        print(Request.fromJson(element));
      });

      List<Request> requests = (jsonDecode(response.body) as List)
  ...
  Future<bool> addRequest(Request request) async {
    try {
      http.Response response = await http
          .post(baseAddress + "/services/tirefitting_RequestService/add",
              headers: {
